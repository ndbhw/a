...
end UP_HDR_GEN_2BAND;

architecture BEHAVE of UP_HDR_GEN_2BAND is

    signal band0_tick               : std_logic_vector(MAX_NUM_PORTC-1 downto 0);
    signal band1_tick               : std_logic_vector(MAX_NUM_PORTC-1 downto 0);
    ...
    component UP_HDR_SEQ_ID_2BAND is
    generic (
        MAX_CH                      : natural := 8
    );
    port (
...
--------------------------------------------------------------------------------
    ...
    begin
        if (CLK'event and CLK = '1') then
            ...
            BAND1_PARAM_INIT_START  <= BAND1_INIT_SESSION;
        end if;
    end process;
    ...
    begin
        if (CLK'event and CLK = '1') then
            ...
            if (BAND1_SECTION_TICK = '1') then
                BAND1_PARAM_PATH_SEL    <= BAND1_PATH_SEL;
            end if;
        end if;
    end process;

    band0_tick                      <= BAND0_PACKING_TICK;
    band1_tick                      <= BAND1_PACKING_TICK;
    id                              <= PARAM_ID;
    ...
    u_HEADER_RTCPC_ID_BAND1 : UP_HDR_RTCPC_ID
    generic map(
        MAX_CH                      => MAX_NUM_PORTC                            --: natural := 2
    )
    port map(
        CLK                         => CLK                                     ,--: in  std_logic;
        RST                         => BAND1_INIT_SESSION                      ,--: in  std_logic;

        PACKING_TICK                => band1_tick                              ,--: in  std_logic_vector(MAX_CH-1 downto 0);
        PACKING_TICK_ACK            => BAND1_PARAM_ORAN_ACK                    ,--: in  std_logic;

        RTCPC_ID                    => id                                      ,--: in  std_logic_array16(MAX_CH-1 downto 0);

        RB_UPDATE                   => BAND1_PARAM_ORAN_START                  ,--: out std_logic;
        RB_eAxC_ID                  => BAND1_PARAM_ORAN_eAxC_ID                 --: out std_logic_vector(15 downto 0)
    );
    ...
    u_HEADER_PE_INDEX_BAND1 : UP_HDR_PE_INDEX
    generic map(
        MAX_CH                      => MAX_NUM_PORTC                            --: natural := 2
    )
    port map(
        ...
        RB_UPDATE                   => open                                    ,--: out std_logic;
        RB_PE_INDEX                 => BAND1_PARAM_ORAN_PE_INDEX                --: out std_logic_vector(2 downto 0)
    );

    u_HEADER_SEQ_ID : UP_HDR_SEQ_ID_2BAND
    generic map(
        MAX_CH                      => MAX_NUM_PORTC                            --: natural := 8
    )
    port map(
        CLK                         => CLK                                     ,--: in  std_logic;
        RST_BAND0                   => BAND0_INIT_SESSION                      ,--: in  std_logic;
        RST_BAND1                   => BAND1_INIT_SESSION                      ,--: in  std_logic;

        BAND0_PACKING_TICK          => band0_tick                              ,--: in  std_logic_vector(MAX_CH-1 downto 0);
        BAND0_PACKING_TICK_ACK      => BAND0_PARAM_ORAN_ACK                    ,--: in  std_logic;
        BAND1_PACKING_TICK          => band1_tick                              ,--: in  std_logic_vector(MAX_CH-1 downto 0);
        BAND1_PACKING_TICK_ACK      => BAND1_PARAM_ORAN_ACK                    ,--: in  std_logic;

        BAND0_RB_SEQUENCE_ID        => BAND0_PARAM_ORAN_SEQUENCE_ID            ,--: out std_logic_vector(15 downto 0);
        BAND1_RB_SEQUENCE_ID        => BAND1_PARAM_ORAN_SEQUENCE_ID             --: out std_logic_vector(15 downto 0)
    );
    ...
    port map(
        ...
        PACKING_TICK                => BAND1_PACKING_TICK                      ,--: in  std_logic_vector(MAX_CH-1 downto 0);
        PACKING_TICK_ACK            => BAND1_PARAM_ORAN_ACK                    ,--: in  std_logic;

        DATA_DIRECTION              => BAND1_DATA_DIRECTION                    ,--: in  std_logic;
        PAYLOAD_VERSION             => BAND1_PAYLOAD_VERSION                   ,--: in  std_logic_vector(2 downto 0);
        FILTER_INDEX                => BAND1_FILTER_INDEX                      ,--: in  std_logic_vector(3 downto 0);
        FRAME_ID                    => BAND1_FRAME_ID                          ,--: in  std_logic_vector(7 downto 0);
        SUBFRAME_ID                 => BAND1_SUBFRAME_ID                       ,--: in  std_logic_vector(3 downto 0);
        SLOT_ID                     => BAND1_SLOT_ID                           ,--: in  std_logic_vector(5 downto 0);
        SYMBOL_ID                   => BAND1_SYMBOL_ID                         ,--: in  std_logic_vector(5 downto 0);

        RB_HEADER_APP               => BAND1_PARAM_ORAN_HEADER                  --: out std_logic_vector(31 downto 0)
    );
    ...
    u_HEADER_SEC_BAND1 : UP_HDR_SEC
    port map(
        CLK                         => CLK                                     ,--: in  std_logic;
        RST                         => BAND1_INIT_SESSION                      ,--: in  std_logic;

        SECTION_TICK                => BAND1_SECTION_TICK                      ,--: in  std_logic;
        SECTION_TICK_ACK            => BAND1_PARAM_SECTION_ACK                 ,--: in  std_logic;

        SECTION_ID                  => BAND1_SECTION_ID                        ,--: in  std_logic_vector(11 downto 0);
        USE_EVERY_PRB               => BAND1_USE_EVERY_PRB                     ,--: in  std_logic;
        START_OF_PRB                => BAND1_START_OF_PRB                      ,--: in  std_logic_vector(9 downto 0);
        NUMBER_OF_PRB               => BAND1_NUMBER_OF_PRB                     ,--: in  std_logic_vector(9 downto 0);

        RB_UPDATE                   => BAND1_PARAM_SECTION_START               ,--: out std_logic;
        RB_HEADER_SEC               => BAND1_PARAM_SECTION_HEADER               --: out std_logic_vector(31 downto 0)
    );
